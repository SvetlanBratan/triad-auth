...
export interface UserContextType {
  ...
  teachings: { value: string, label: string }[];
  ...
  adminClaimHuntEarly: (ownerUserId: string, characterId: string, huntId: string) => Promise<InventoryItem[]>;
  adminStartHunt: (userId: string, characterId: string, familiarId: string, locationId: string) => Promise<void>;
  adminStartMultipleHunts: (userId: string, characterId: string, familiarIds: string[], locationId: string) => Promise<void>;
...
const FREE_CHARACTER_SLOTS = 6;

const isInactiveUser = (user: Pick<User, 'status'>) => user.status === 'неактивный';
...
export function UserProvider({ children }: { children: React.ReactNode }) {
    ...
    }, [currentUser?.id, fetchUserById, setCurrentUser]);
    ...
    useEffect(() => {
        ...
        const dateUnsubscribe = onValue(dateRef, 
            (snapshot) => {
    ...
    const updateCharacterInUser = useCallback(async (userId: string, characterToUpdate: Character): Promise<User> => {
        await runTransaction(db, async (transaction) => {
            ...
            const totalCharacterSlots = FREE_CHARACTER_SLOTS + (sourceUserData.extraCharacterSlots || 0);

            const updatesToApply: Partial<User> = {};

            if (isNewCharacter) {
                if (updatedCharacters.length >= totalCharacterSlots) {
                    throw new Error(`Достигнут лимит персонажей: ${updatedCharacters.length}/${totalCharacterSlots}. Чтобы открыть слот 7+, купите награду «Дополнительный персонаж».`);
                }
 ...
 const fetchOpenExchangeRequests = useCallback(async (): Promise<ExchangeRequest[]> => {
  ...
  const purchaseShopItem = useCallback(async (shopId: string, itemId: string, buyerUserId: string, buyerCharacterId: string, quantity: number) => {
    ...
    const processAnnualTaxes = useCallback(async (): Promise<{ taxedCharactersCount: number; totalTaxesCollected: BankAccount }> => {
        ...
        for (const user of allUsers) {
            ...
            const updatedCharacters = user.characters.map(character => {
                ...
                // 2. Trade Tax
                const ownedShops = allShopsData.filter(shop => shop.ownerCharacterId === character.id);
    ...
    const adminClaimHuntEarly = useCallback(async (ownerUserId: string, characterId: string, huntId: string): Promise<InventoryItem[]> => {
        ...
        if (updatedUser && currentUser?.id === ownerUserId) {
            setCurrentUser(updatedUser);
        }
        
        return rewards;
    }, [currentUser?.role, fetchAllShops, gameSettings, familiarsById, fetchUserById, setCurrentUser]);

    const adminStartHunt = useCallback(async (userId: string, characterId: string, familiarId: string, locationId: string) => {
        const location = gameSettings.huntingLocations?.find(l => l.id === locationId);
        if (!location) throw new Error("Location not found");
        ...
        const character = targetUser.characters.find(c => c.id === characterId);
        if (!character) throw new Error("Character not found");
        ...
        await runTransaction(db, async (transaction) => {
            const userRef = doc(db, "users", userId);
            const userDoc = await transaction.get(userRef);
            if(!userDoc.exists()) throw new Error("User doc not found in transaction");
            const userData = userDoc.data() as User;
            const charIndex = userData.characters.findIndex(c => c.id === characterId);
            if(charIndex === -1) throw new Error("Character not found in transaction");
    ...
    }, [gameSettings.huntingLocations, fetchUserById, currentUser?.id, setCurrentUser]);

    const adminStartMultipleHunts = useCallback(async (userId: string, characterId: string, familiarIds: string[], locationId: string) => {
        const location = gameSettings.huntingLocations?.find(l => l.id === locationId);
        if (!location) throw new Error("Location not found");
    ...
    const adminAddShop = useCallback(async (shopData: Omit<Shop, 'id' | 'items' | 'purchaseCount' | 'aiHint'>) => {
        ...
        const newShop: Partial<Shop> = { id: newShopId, ...shopData, items: [] };
