...
const CharacterDisplay = ({ character, onDelete }: { character: Character, onDelete: (characterId: string) => void }) => {
    ...
    return (
        <AccordionItem value={character.id} className="border rounded-md px-2 mb-2 hover:bg-muted/50 dark:hover:bg-muted transition-colors duration-200">
     ...
     <div className="space-y-6">
        <Card>
          ...
          <CardContent className="overflow-x-auto p-0">
             <ScrollArea className={cn("px-6", showAllPointsHistory && sortedPointHistory.length > 5 ? "h-96" : "h-auto")}>
                <Table>
                <TableHeader>
                    <TableRow>
